...
> **状态：代理式架构已落地。** 引擎、插件、桌面应用都可运行，**245 个测试通过**，
> 界面冒烟与真实加密落盘均已验证。见文末「已知限制」。
...
覆盖：HTML→文本抽取（含升级—回退链与闸门）、配置合并与校验、插件清单校验（含坏样本）、
DuckDuckGo 解析、DeepSeek 响应映射、模型 JSON 校验与重试、provider 原生工具调用线格式、
**黑板与不动点运行器**（地图只增不减、任务互相重新激活、收敛/卡死/撞护栏三种收尾）、
**搜索任务的饱和终止**、**大纲任务的覆盖度判据与缺口请求**、**写作任务的工具循环与自检**、
**话题缓存**（id 稳定性、损坏容错、原子写入）、
...
- **代理式主流程需要大模型 key，且我无法在无 key 环境下实测它的真实输出质量**。
  机制层面有端到端测试覆盖（用可编排的假模型跑通采集→归纳→大纲→工具循环写作→输出、
  引用链完整性、饱和终止、话题复用、降级链），但**提示词在真实模型上的效果没有验证过**。
  这是本次改动里最大的未验证面，我不想含糊过去。
