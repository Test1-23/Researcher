# 关于「演示：Alpha 与 Beta 是什么关系」的资料汇编

> **查询**：演示：Alpha 与 Beta 是什么关系
> **生成时间**：2026-09-10T04:18:07.856Z（用时 15 ms）
> **整理方式**：organize-extractive
> **搜索方式**：search-duckduckgo（主搜索插件不可用，已降级）
> **降级说明**：配置的整理插件 organize-llm 不可用，本次改用 organize-extractive

## 摘要

围绕「演示：Alpha 与 Beta 是什么关系」共收集到 2 个来源，成功抓取正文 2 篇。以下各节由「抽取式整理」生成：直接摘录各来源的开头若干句，未经大模型改写或概括，因此不代表对来源内容的判断，请以原文为准。

## Alpha 文档

Alpha 是一个用于演示的示例项目，它提供基础能力并保持接口稳定。 安装 Alpha 只需要一条命令，随后即可开始使用。 Alpha 的配置集中在单个文件里，便于迁移与备份。

**引用**：[#1](https://example.com/alpha)

## Beta 指南

Beta 的入门指南介绍了安装步骤与常见问题，适合第一次接触的用户阅读。 指南同时给出了若干示例，帮助读者快速理解核心概念。

**引用**：[#2](https://example.com/beta)

## 来源

1. [Alpha 官方文档](https://example.com/alpha) — 已抓取正文 86 字
2. [Beta 入门指南](https://example.com/beta) — 已抓取正文 61 字

## 运行信息

- 引擎版本：0.1.0
- 主流程：pipeline-default
- 搜索插件：search-duckduckgo
- 大模型插件：provider-openai
- 整理插件：organize-extractive
- 输出插件：output-markdown、output-html
- run id：2026-09-10T04-18-07-851Z-s6vpxq

> 本报告的正文由上述整理插件生成。引用编号对应「来源」列表；大模型整理可能概括失准，关键结论请点开来源原文核对。
